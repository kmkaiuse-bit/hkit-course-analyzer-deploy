...
const SubjectCollector = {
    ...
    extractFromTranscript(transcriptText) {
        ...
        const patterns = [
            // Simple format: "Course Name    Grade" (your transcript format)  
            /^\s*([A-Z][^A-F]*?)\s+[A-F][+-]?\s*$/gm,
    ...
    /**
     * Add multiple subjects at once (Stage 1 integration)
     * @param {Array} subjectArray - Array of subject names to add
     * @returns {number} Number of subjects added
     */
    addBulkSubjects(subjectArray) {
        if (!Array.isArray(subjectArray)) return 0;
        ...
        if (addedCount > 0) {
            this.saveToStorage();
            console.log(`🎯 Bulk added ${addedCount} subjects to dropdown (${beforeCount} → ${this.extractedSubjects.size})`);
        }
    ...
    /**
     * Clear cache and reset to clean state (fixes dropdown preset issue)
     */
    clearCache() {
        ...
        // Also clear any old cache keys that might exist
        const oldKeys = ['commonSubjects', 'hkit_subjects', 'collected_subjects'];
        oldKeys.forEach(key => localStorage.removeItem(key));
        
        console.log('🔄 Cache cleared completely - dropdown will now show only extracted subjects');
        
        // Show notification if NotificationManager is available
        if (typeof NotificationManager !== 'undefined') {
            NotificationManager.show('✅ Cache cleared successfully! Dropdown reset to show only extracted subjects.', 'success');
        }
    },
    ...
    getCacheInfo() {
        ...
        return {
            cacheExists: !!stored,
            subjectCount: this.extractedSubjects.size,
            lastUpdated: stored ? JSON.parse(stored).lastUpdated : null,
            cacheSize: stored ? stored.length : 0
        };
    },
